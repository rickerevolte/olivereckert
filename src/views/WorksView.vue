<template>
  <div v-if="!this.cookieAccepted" id="myCookieConsent" class="cookieConsent">
    <CookieConsent
      ...
      @closePopUpClicked="hideCookieWindow()"
    ></CookieConsent>
  </div>
  <div v-else>
    <!-- <h2>Welcome Cookies!</h2> -->
  </div>
  <h3 class="white">works</h3>
  ...
  <div id="userSortAndFilter">
    <label>
      <select
        id="selectSort"
        v-model="selected"
        @change="this.sort(this.filteredFilms)"
      >
        <div>
          <p class="white">{{ selected }}</p>
        </div>
    ...
    <label>
      <p class="white">filter by: <input @input="this.sortAndFilter()" /></p>
    </label>
  </div>
  <!---->
  <div id="picGaleryContainer">
    <div id="picGalery">
      <VideoPic
        v-for="(film, index) in this.sortedAndFilteredFilms"
  ...
  <div id="myModal" class="modal">
    ...
    <div class="modal-content">
      <VimeoPlayer
        v-if="!this.playerClosed"
        :this.vimeoID="'785975822'"
      ...
      <div class="filmInfo">
        ...
        <div id="fundingBox">
          ...
          <h5>{{ this.selectedFilm.fundedText }}</h5>
        </div>
      ...
      >
        <span class="close" @click="this.doSomething('closing player')"
          >&times;</span
        >
      </div>
    </div>
  ...
  methods: {
    hideCookieWindow() {
      ...
      localStorage.setItem(
        "cookieConsent",
        JSON.stringify(this.cookieAccepted)
      );
    },
